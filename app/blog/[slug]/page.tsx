...
export default async function BlogPostPage({ params }: Props) {
  ...
  return <><JsonLd data={pageSchema(tenant, post)} /><header className="page-hero"><Image src={post.featuredImage.src} alt={post.featuredImage.alt} fill loading="eager" fetchPriority="high" sizes="100vw" /><div className="container page-hero__content"><div className="page-hero__frame"><Breadcrumbs current={post.name} parent={{ label: "Painting guide", href: "/blog" }} /><p className="eyebrow">{new Date(post.publishedAt).toLocaleDateString("en-US", { dateStyle: "long" })} · {post.author.name}</p><h1>{post.seo.h1}</h1><p className="lede">{post.excerpt}</p></div></div></header><article className="section article-section"><div className="container article-layout"><div className="prose article-copy">{post.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><aside className="content-aside article-aside"><p className="eyebrow">Continue planning</p><h2>Related resources</h2><ul className="link-list">{service ? <li><Link href={`/${service.slug}`}>{service.name}</Link></li> : null}{location ? <li><Link href={`/${location.slug}`}>Painting in {location.city}</Link></li> : null}{project ? <li><Link href={`/projects/${project.slug}`}>{project.name}</Link></li> : null}</ul></aside></div></article><CallToAction tenant={tenant} heading={post.cta} /></>;
}
