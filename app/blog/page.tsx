...
export default async function BlogIndexPage() {
  const tenant = await getCurrentTenant();
  return <><header className="page-hero"><div className="container"><p className="eyebrow">Painting guide</p><h1>Practical questions before paint day</h1><p className="lede">Demonstration articles structured for first-hand painter advice, real product observations, and useful internal links.</p></div></header><section className="section"><div className="container grid-3">{tenant.posts.filter((post) => post.status === "published").map((post) => <article className="card" key={post.id}><Link className="card-link" href={`/blog/${post.slug}`}><div className="card-image"><Image src={post.featuredImage.src} alt={post.featuredImage.alt} fill sizes="(max-width: 620px) 100vw, 33vw" /></div><div className="card__body"><p className="eyebrow">Updated {new Date(post.updatedAt).toLocaleDateString("en-US", { dateStyle: "medium" })}</p><h2 style={{ fontSize: "1.65rem" }}>{post.name}</h2><p>{post.excerpt}</p><strong>Read the guide</strong></div></Link></article>)}</div></section></>;
}
